...
def construct_query(customer_query):
    ...
    if location:
        expression_parts.append("room_location = :location")
        expression_values[":location"] = location

    if price_range:
        expression_parts.append("room_price BETWEEN :min_price AND :max_price")
        expression_values[":min_price"] = price_range[0]
        expression_values[":max_price"] = price_range[1]

    if room_type:
        expression_parts.append("room_type = :room_type")
        expression_values[":room_type"] = room_type

    if amenities:
        for i, amenity in enumerate(amenities):
            expression_parts.append(f"contains(amenities, :amenity{i})")
